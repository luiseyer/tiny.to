<script lang="ts">
  ...
  import { getClicksState } from '$lib/clicks/state.svelte'
  import {
    Card,
    CardContent,
    CardDescription,
...
<div class="flex flex-col gap-2 px-4 lg:px-6">
  <Card>
    <CardHeader>
      ...
      <CardDescription>Enlaces mas visitados en el último mes.</CardDescription>
    </CardHeader>
    <CardContent>
        ...
        <Table>
          ...
          <TableBody>
            {#each topLinks as item (item.id)}
              <TableRow>
                <TableCell class="max-w-24 overflow-hidden px-8 py-4 text-ellipsis">
                ...
                <TableCell class="px-8 py-4 text-right">
                  {Intl.NumberFormat('es', { compactDisplay: 'short', notation: 'compact' }).format(
                    item.clicks * 10
                  )}
                </TableCell>
              </TableRow>
